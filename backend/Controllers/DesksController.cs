    ...
    public class DesksController : ControllerBase
    {
       private readonly DataContext _repo;
        ...
        public async Task<IActionResult> Put(int id, [FromBody] Desk deskDto)
        {
            ...
            if(desk != null)
            {
            desk.FirstName = deskDto.FirstName;
            desk.LastName = deskDto.LastName;
            desk.NumberDesk = deskDto.NumberDesk;
            desk.IsCoordinator = deskDto.IsCoordinator;
            desk.IsVolunteer = deskDto.IsVolunteer;
            desk.Team = deskDto.Team;
            _repo.Desks.Update(desk);
            await _repo.SaveChangesAsync();
